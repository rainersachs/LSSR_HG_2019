...
# Filename:     plots.R 
# Purpose:      Contains code to generate the LSSR paper figures. It is
#               part of the source code for Harderian Gland LSSR project.
...
#============================================================#
ddose <- 0:701 
# RKS: To force 7 ticks on y axis, see par arguments to plot().
plot(c(0, 701), c(-.02, .75), pch = 19, col = 'white', ann = FALSE, bty = 'u') 
...
errbar(dat_down[dat_down$Z == 2, ][, "dose"], 
       ...
       yplus  = dat_down[dat_down$Z == 2, ][, "Prev"] 
                + dat_down[dat_down$Z == 2, ][, "SD"], 
       yminus = dat_down[dat_down$Z == 2, ][, "Prev"] 
                - dat_down[dat_down$Z == 2, ][, "SD"], 
       pch = 19, cap = 0.02, add = TRUE, col = 'red', 
       errbar.col = 'red', lwd = 1) # Proton data
legend(x = "bottomright", legend = "SD", cex = 0.6)

# Alpha particle data
errbar(dat_down[dat_down$Z == 1, ][, "dose"], 
       dat_down[dat_down$Z == 1, ][, "Prev"],
       yplus  = dat_down[dat_down$Z == 1, ][, "Prev"] 
                + dat_down[dat_down$Z == 1, ][, "SD"], 
       yminus = dat_down[dat_down$Z == 1, ][, "Prev"] 
                - dat_down[dat_down$Z == 1, ][, "SD"], 
       pch = 19, cap = 0.02, add = TRUE, col = 'black', 
       errbar.col = 'black', lwd = 1) # Proton data
legend(x = "bottomright", legend = "SD", cex = 0.6)
...
errbar(dat_down[dat_down$LET == 193,][,"dose"], 
       dat_down[dat_down$LET == 193,][,"Prev"],
       yplus  = dat_down[dat_down$LET == 193, ][, "Prev"] 
                + dat_down[dat_down$LET == 193, ][, "SD"], 
       yminus = dat_down[dat_down$LET == 193, ][, "Prev"] 
                - dat_down[dat_down$LET == 193, ][, "SD"],
       pch = 19, cap = 0.04, add = TRUE, col = 'black',
       errbar.col = 'black', lwd = 1)
legend(x = "topleft", 
       legend = paste("Fe 600, not 95% CI, SD, Y_0 = ", Y_0), cex = 0.49)
...
errbar(dat_down[dat_down$LET ==70,][,"dose"], 
       dat_down[dat_down$LET ==70,][,"Prev"],
       yplus  = dat_down[dat_down$LET == 70, ][, "Prev"] 
                + dat_down[dat_down$LET == 70, ][, "SD"], 
       yminus = dat_down[dat_down$LET == 70, ][, "Prev"] 
                - dat_down[dat_down$LET == 70, ][, "SD"],
       pch = 19, cap = 0.04, add = TRUE, col = 'black', 
       errbar.col = 'black', lwd = 1)
legend(x = "topleft", 
       legend = paste("Si, not 95% CI, SD, Y_0 = ", Y_0), cex = 0.49)
...
d5_p <- c(0.01 * 0:9, 0.1 * 1:9, 1:40)
     ...
     #===================================================#
        ...
        uncorr_fig_5A <- simulate_monte_carlo(n = 500, d5_new, LET_vals, ratios,
...
errbar(70, Prev[3], yplus = Prev[3] +  SD[3], yminus = Prev[3] - SD[3],
...
Prev <- as.numeric(mix_data$Prev) - Y_0
SD <- as.numeric(mix_data$SD)
errbar(40, Prev[5], yplus = Prev[5] + SD[5], yminus = Prev[5] - SD[5],
       pch = 19, cap = 0.05, add = TRUE, col = 'black', 
       errbar.col = 'black', lwd = 1)
...
Prev <- as.numeric(mix_data$Prev) - Y_0
SD <- as.numeric(mix_data$SD)
errbar(40, Prev[6], yplus = Prev[6] + SD[6], yminus = Prev[6] - SD[6],
       pch = 19, cap = 0.05, add = TRUE, col = 'black',
       errbar.col = 'black', lwd = 1)
